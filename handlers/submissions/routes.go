...
func (h *Handler) RegisterRoutes(mux *http.ServeMux, manager *middlewares.Manager, middlewares *middlewares.Middlewares) {
	...
	mux.Handle("PATCH /api/submissions", manager.With(h.UpdateSubmission, middlewares.AuthEngine))
	mux.Handle("GET /api/submissions/{submissionId}", manager.With(h.GetSubmission, middlewares.Authenticate))
